...
function showBingeIndicator(status) {
  ...
  function render() {
    ...
    widget.innerHTML =
      ...
      '<span class="adb-binge-icon">' +
      (critical ? "\u26A0\uFE0F" : "\u23F1\uFE0F") +
      "</span>" +
      '<span class="adb-binge-domain">' +
      status.domain +
      "</span>" +
      '<span class="adb-binge-close" id="adb-binge-close">\u00D7</span>' +
      "</div>" +
      '<div class="adb-binge-bar-wrap">' +
      '<div class="adb-binge-bar-fill" style="width:' +
      Math.round(pct * 100) +
      '%"></div>' +
      "</div>" +
      '<div class="adb-binge-footer">' +
      '<span class="adb-binge-used">' +
      fmt(usedSecs) +
      " / " +
      fmt(limitSecs) +
      " used</span>" +
      '<span class="adb-binge-left ' +
      (critical ? "red" : "") +
      '">' +
      (remainSecs > 0 ? fmt(remainSecs) + " left" : "Limit reached") +
      "</span>" +
      "</div>";
  }
...
function goBack() {
  chrome.runtime.sendMessage({ type: "STAGE_RESET" });
  removeOverlay();
  removeCountdownBadge();
  history.back();
}
...
function showStage(stage, bingeStatus) {
  ...
  if (stage === 1) {
    playAlert("warning");
    var overlay = createOverlay("adb-stage-1");
    overlay.innerHTML =
      '<div class="adb-card">' +
      ...
      '<button class="adb-btn adb-btn-ghost" id="adb-snooze">Snooze 5 min</button>' +
      '<button class="adb-btn adb-btn-ghost" id="adb-allow-once">Allow once</button>' +
    ...
    document
      .getElementById("adb-snooze")
      .addEventListener("click", function () {
        chrome.runtime.sendMessage(
          { type: "PAUSE_BLOCKING", minutes: 5 },
          function () {
            removeOverlay();
            removeCountdownBadge();
            chrome.runtime.sendMessage({ type: "STAGE_RESET" });
    ...
    document
      .getElementById("adb-allow-once")
      .addEventListener("click", function () {
        chrome.runtime.sendMessage({ type: "ALLOW_ONCE" }, function () {
          removeOverlay();
          removeCountdownBadge();
        });
      });
    document
      .getElementById("adb-continue")
      .addEventListener("click", function () {
        removeOverlay();
        showCountdownBadge(20, function () {
          chrome.runtime.sendMessage({ type: "STAGE_ADVANCE" }, function (res) {
    ...
    playAlert("final");
    var overlay = createOverlay("adb-stage-2");
    overlay.innerHTML =
      '<div class="adb-card">' +
    ...
    var overlay = createOverlay("adb-stage-3");
    ...
    overlay.innerHTML =
      ...
      "</p>" +
      unblockText +
      '<button class="adb-btn-exit" style={{padding:"12px"}} id="adb-exit">Leave This Website</button>' +
      "</div>";
    document.body.appendChild(overlay);
    ...
    exitBtn.addEventListener("click", function () {
      chrome.runtime.sendMessage({ type: "STAT_INCREMENT", field: "exited" });
      document.body.style.overflow = "";
      history.back();
    });
  }
...
function startBingeHeartbeat(url) {
  ...
  // Send heartbeat every 30s — background records 30s of usage and checks if blocked
  bingeHeartbeatInterval = setInterval(function () {
    if (!isDistractingPage) {
      stopBingeHeartbeat();
      return;
    }
    chrome.runtime.sendMessage(
      { type: "BINGE_HEARTBEAT", url: url, secs: 30 },
      function (res) {
